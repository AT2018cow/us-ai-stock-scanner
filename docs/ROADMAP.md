...
Phase 4 数据揭示 balanced 与 risk_off 收益相关性 0.87（月度）/0.999（季度）、down 时段逐位相同——"三风格"实为"2 个真实独立风格 + 1 个冗余副本"。同时 QQQ 年度收益逐年递减（+45.8%→+26.7%→+19.1%→+7.3%），AI 超级牛市基准不可外推。**用户决策**：

1. 两风格架构：risk_on（双动量进攻）+ risk_off（低吸防守 + SMA200 深度熊市熔断）；
...
## Phase 4 体系级验证记录（2026-09-24，D4 判定）

**执行**：三个 promote 后配置在 Modal 三容器并行完整重放（monthly × 4 窗口 × 3 场景，`phase4_validation.py`，~$0.25），产物 `outputs/phase4_{balanced,risk_on,risk_off}_{events,benchmarks,segments}.csv`。

**口径敏感性的重大发现**：20 天持有口径下三风格全部亏损（组合 −5.3% vs QQQ +121.9%）——因三风格信号设计假设 60-120 天展开，短持有系统性截断收益。**60 天 × 季度非重叠是正确口径**，此后所有体系评估必须使用它（运营时若按 20 天持有执行会摧毁体系价值）。

**60 天季度非重叠的结果矩阵**：

| 年度 | balanced | risk_on | risk_off | QQQ | 当年最佳 |
|---|---|---|---|---|---|
| 2023 | +22.9% | −12.1% | +22.9% | +45.8% | balanced |
| 2024 | +28.0% | +29.5% | +28.0% | +26.7% | risk_on |
| 2025 | −11.6% | +10.3% | −13.8% | +19.1% | risk_on |
| 2026 | +40.3% | +4.8% | +40.3% | +7.3% | balanced |

| 组合方式 | 累计 | 年化 | 最大回撤 | 判定 |
|---|---|---|---|---|
| 等权三风格 | +79.2% | +16.8% | −16.7% | 跑输 QQQ（+136.1%），回撤更深 |
| Oracle 年度轮动（上限） | +146.3% | — | −23.6% | 勉强跑赢，回撤远深于 QQQ（−7.4%） |
| QQQ 基准 | +136.1% | +27.8% | −7.4% | AI 超级牛市窗口的极端基准 |

**D4 判定：原标准（跑赢 QQQ 且回撤更浅）未通过**。但体系机制层的核心假设被证实：
1. **风格×regime 匹配全部方向正确**：动量年（2024/2025）risk_on 最佳，回调年（2023/2026）balanced/risk_off 最佳——轮动信号真实存在；
2. 各风格绝对收益健康（balanced 年化 +19.5%、risk_off +18.7%、risk_on 被 2023 拖累至 +7.6%）；
3. 等权（+79%）与 oracle（+146%）之间有 67 个点的**轮动 alpha 空间**——等权是体系的下限，权重轮动是明确的改进方向。

**处置选项（待用户决策）**：
- A. 修订体系定位：从"跑赢 QQQ"改为"绝对收益型全天候配置"（年化 17-19%，代价是牛市落后）——接受 D4，进入 Phase 5 运营；
- B. 迭代轮动机制：用 regime 信号（QQQ 60 日动量）做风格权重动态分配，目标捕获等权→oracle 之间的部分轮动 alpha——新增一轮工程（估计 2-3 天 + 调参）；
- C. 两者结合：按 A 定位进入运营，轮动机制作为 Phase 5 的持续改进项。

## Phase 4 补充验证（2026-09-24，规则轮动实证）

用户追问 oracle 概念触发第三轮验证，**修复一个引擎 bug**（`b5c509c`）：balanced 配置的 bars 拉取从未包含 QQQ（regime symbol 仅经 `benchmark_trend_filter_symbol` 加入，balanced 该值为 null）→ 1584/1584 事件 regime=unknown。修复后 regime symbol 恒进入 bars 拉取。影响评估：Phase 3 balanced 候选回退旧公式（设计内行为，promote 决策基于 balanced_rank_score 不受影响）；risk_on/risk_off 带 filter 不受影响。

**修正后的规则轮动实证（60 天季度非重叠，regime 本地补算）**：

| 组合 | 累计 | 年化 | 回撤 | 可实现性 |
|---|---|---|---|---|
| 等权三风格 | +79.2% | +16.8% | −16.7% | ✓ |
| **规则轮动（QQQ 动量 up→risk_on / down→balanced）** | **+63.8%** | +14.1% | −23.6% | ✓ 但**劣于等权** |
| Oracle 年度轮动 | +146.3% | — | −23.6% | ✗ 事后上限 |
| QQQ | +136.1% | +27.8% | −7.4% | 基准 |

**关键反面发现：简单 regime 信号无法实现轮动 alpha**——QQQ 动量与风格相对收益的相关性不足（2023：QQQ up 但 risk_on 亏 −12.1%，应选 balanced；2026：QQQ up 但 balanced +40.3% 远超 risk_on +4.8%）。选项 B 的期望值据此**下调**：轮动信号需要远比 QQQ 动量复杂的构造（风格相对动量、拥挤度、估值差等），投入产出比不确定。

**修订后的处置建议**：倾向 **C**——按"绝对收益型全天候配置"定位进入 Phase 5 运营（等权组合年化 16.8% 是当下稳健的可实现基线），把轮动信号研究列为远期课题（先记录 oracle 67 点空间与简单信号证伪的证据链）。

## Phase 4 准确性审查（2026-09-24，用户质量关卡）

| 审查项 | 方法 | 结果 |
|---|---|---|
| 事件完整性 | 每月每清单每 horizon 行数/重复检查（9 组合 × 44 期） | ✓ 0 重复，结构完整 |
| QQQ 前瞻收益 | 引擎语义手工复算（next_open 入场 + 入场日计入持有序 + benchmark cost=0） | ✓ 逐位吻合（此前 0.6-1.35% 表观差全部为口径差：入场价模式、持有序列基准、基准免成本） |
| segments vs events 聚合 | 2025 balanced/60d 手工均值对照 | ✓ 差 0.000000 |
| 等权组合累计 | 独立脚本从原始 events 重算 15 季度点 | ✓ +79.16% 复现 |
| Oracle 年度轮动 | 独立重算 + 各年最佳风格 | ✓ +146.32% 复现，风格归属一致 |
| **规则轮动 regime 标记** | 本地补算 vs 云端原生对照 | ✗ **发现分析错误**：本地补算 `Timestamp(date, 00:00)` 截断排除了信号日当天 close（off-by-one），2024-04-30 与 2026-01-30 两个符号边界日期被翻转 |

**修正**：规则轮动改用云端原生 regime（risk_on events 自带，含当天的正确回测语义）。**规则轮动最终数字从 +63.8% 下修至 +52.9%**——证伪结论增强（劣于等权 79.2% 达 26 个点）。等权与 oracle 的 headline 数字不受影响（独立复算双双吻合）。

**引擎判定：Phase 4 引擎与回测计算无错误**；错误发生在审查者（AI 助手）的临时分析脚本层面。教训（第三例）：凡是临时补算分析，必须与既有原生数据源做一致性对照后才可用于结论。
