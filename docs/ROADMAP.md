...
> 状态：Phase 1 引擎与配置改造完成，D1 首验未通过（risk_on 需迭代），待决策 A' 或 B 路线
> 最后更新：2026-09-22
> 本文档是 risk_on / risk_off / balanced 三风格体系改造与调参的权威计划。

## Phase 1 进展记录（2026-09-22）
...
**D1 首验结果（monthly × 4 窗口，risk_on 改造前后对比，60 天超额 vs QQQ）**：

| 年度 | risk_on 基线 | risk_on 改造后 | 判读 |
|---|---|---|---|
| 2023 | −5.1% | −2.8% | 略改善 |
| 2024 | +0.8% | −3.1% | 恶化 |
...
**根因（三项）**：
1. **估值-动量冲突**：risk_on 镜像了结构参数，但保留了 balanced 的全部估值硬门槛（`max_ps_hist_percentile`、SIC 分位等）。2025 年动量强势股（AMD PE 79、OKTA PE 45 类）几乎全被估值过滤杀掉——趋势确认与估值约束的交集在 44 期里只剩 15 个信号（29 期空仓）。
2. **空仓机会成本**：评估口径按"每期 vs QQQ"，66% 空仓期在 QQQ 上涨年自动累积负超额（2025 年 2-10 月连续 9 期空仓，正是普涨段）。
3. **风格方向正确但表达能力不足**：选出的确是趋势股（KLAC/TXN/AMAT/MU/ACN），说明结构镜像生效；问题在"进攻风格应容忍估值"这一维度未同步。

**决策点（待用户确认）**：
- **A'（推荐先试，成本最低）**：保持 A 结构，手工放宽 risk_on 的估值门槛（`max_ps_hist_percentile`→null 或 0.95、SIC 分位上限放开）——进攻风格以趋势换估值容忍；重跑 D1（~1.5h）。
- **B 路线**：momentum 清单升级为 risk_on 的 primary（momentum 清单本就不走估值硬过滤，设计意图即趋势跟随）。
- 另注意：空仓机会成本是评分口径问题，Phase 2 的 regime 条件评分需纳入"QQQ 上涨期参与度"指标。
