...
- `cache/` and `outputs/` are gitignored. Alpaca cache has TTLs (snapshots 120s); **SEC cache (`cache/submissions_*`, `cache/facts_*`) has NO TTL — stale until deleted**. For genuinely fresh fundamentals: `rm cache/submissions_*.json cache/facts_*.json` before scanning. SEC is rate-limited (~5 req/s); use `--max-symbols N` for quick trials.
